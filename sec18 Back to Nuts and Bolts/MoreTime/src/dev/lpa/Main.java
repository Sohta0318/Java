...
public class Main {
    public static void main(String[] args) {
        ...
        for(ZoneId z : List.of(
            ...
            Instant dobInstant = Instant.parse("2022-01-01T08:01:00Z");
            ...
            ZonedDateTime donHere = dobSydney.withZoneSameInstant(ZoneId.systemDefault());
            System.out.println("Your kid's birthdate, Here time = " + donHere.format(
                    DateTimeFormatter.ofLocalizedTime(FormatStyle.MEDIUM)));
            ...
            for(ChronoUnit c : ChronoUnit.values()){
                if(c.isSupportedBy(LocalDate.EPOCH)){
                    long val = c.between(LocalDate.EPOCH,
                            dob2.toLocalDate());
                    System.out.println(c + " past = " + val);
                }else{
                    System.out.println("-- Not supported: " + c);
                }
            }
        ...
        for(ChronoUnit c : ChronoUnit.values()){
            if(c.isSupportedBy(ldt)){
                long val = c.between(ldt,
                        dob2);
                System.out.println(c + " past = " + val);
            }else{
                System.out.println("-- Not supported: " + c);
