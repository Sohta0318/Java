...
public class ContactData {
    ...
    public static List<Contact> getData(String type){
        List<Contact> datlist = new ArrayList<>();
        Scanner scanner = new Scanner(type == "phone" ? phoneData : emailData);
        while (scanner.hasNext()){
            String[] data = scanner.nextLine().split(",");
            Arrays.asList(data).replaceAll(String::trim);
            if(type.equals("phone")){
                datlist.add(new Contact(data[0], Long.parseLong(data[1])));
            } else if (type.equals("email")) {
                datlist.add(new Contact(data[0], data[1]));
            }
        }
        return datlist;
